...
// const getAddJobsListDownstream = async (jobsList, ci) => {
//   let downstreamJobs = [];

//   try {
//     const workspaceSet = jobsList.reduce(
//       (acc, val) => ({ ...acc, [val.upstreamjobCiWorkspace]: null }),
//       {}
//     );
//     const workspaceLists = Object.keys(workspaceSet);

//     for (let i = 0; i < workspaceLists.length; i++) {
//       workspaceSet[workspaceLists[i]] = await getSsoToken(workspaceLists[i]);
//     }

//     for (let i = 0; i < jobsList.length; i++) {
//       const api_url = `http://${process.env.USER_ID}:${
//         workspaceSet[jobsList[i].upstreamjobCiWorkspace]
//       }@${jobsList[i].jobName.replace("http://", "")}api/json?tree=buildable`;
//       const response = await handleFetch(api_url);
//       const responseBody = await response.json();
//       const is_also_deployment = genericUtils.isDeploymentJob(
//         jobsList[i].jobName
//       );
//       const { buildable } = responseBody;
//       let jobObj = [
//         jobsList[i].jobName,
//         jobsList[i].upstreamJobName,
//         jobsList[i].upstreamjobCiWorkspace,
//         jobsList[i].clientName,
//         jobsList[i].isPipeline,
//         ci,
//         is_also_deployment,
//         buildable,
//       ];
//       downstreamJobs = [...downstreamJobs, jobObj];
//     }
//     return downstreamJobs;
//   } catch (err) {
//     logger.debug(
//       `jobworkers(getAddJobsListDownstream) : Failed due to error ${err.stack}`,
//       4,
//       0
//     );
//     throw err;
...
const addTeamCiTenant = async (payload) => {
  try {
    ...
    let insertQueries = [];
    console.log("return types", ciExist, teamExist, tenantExist);
    if (!ciExist || !ciExist.length) {
      insertQueries.push(PG_QUERY_EXECUTORS.insertCiQuery([[ci]]));
    }

    if (!teamExist || !teamExist.length) {
      insertQueries.push(PG_QUERY_EXECUTORS.insertTeamsQuery([[team]]));
    }

    if (!tenantExist || !tenantExist.length) {
      insertQueries.push(
    ...
    await PG_QUERY_EXECUTORS.populateTeamCiTenant(querySet);
  } catch (err) {
    logger.debug(
...
const getBuildAverges = async (tenant) => {
  return PG_QUERY_EXECUTORS.fetchBuildAvgs(tenant);
};

module.exports = {
  // getAddJobsListDownstream,
